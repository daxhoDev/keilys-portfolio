...
- `favicon.svg` · `favicon-96.png` · `apple-touch-icon.png` (180 × 180). Generated by
  `npm run favicons`, which extracts the **K** outline from the Playfair woff2 the site already ships and
  bakes it into the SVG as a path — setting `<text>` would render in whatever serif the build machine has.
- **The mark is a disc**, not a square: the browser icons have transparent corners.
- **`apple-touch-icon.png` stays square.** iOS applies its own squircle mask, so a round icon with
  transparent corners lands as a circle floating on a black backdrop. The platform rounds it; doing it twice
