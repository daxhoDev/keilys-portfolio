...
| 5.10 | Branching | **One long-lived `development` branch**; `master` untouched until launch | Vercel's Production Branch stays `master`, so the production URL is empty during the build and the branch alias is what gets shared. Merging at [Phase 10](./11-implementation-plan.md#phase-10--launch) is what puts the site live |
